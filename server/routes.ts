...
요청 시간: ${new Date().toLocaleString('ko-KR')}
      ...
      if (transporter) {
        try {
          ...
          const recipients = ['daehyun.jee@rethinkwealth.com'];
          ...
          const info = await transporter.sendMail(mailOptions);
          console.log('✅ Consultation email sent successfully to daehyun.jee@rethinkwealth.com');
          console.log('Message ID:', info.messageId);
        } catch (emailError) {
          console.error('❌ Failed to send consultation email:', emailError);
          console.log('📧 Consultation Email Content (logged as backup):');
          console.log('To: daehyun.jee@rethinkwealth.com');
